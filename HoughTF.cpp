...
void HTLineDetection(unsigned char* BinaryImageDst, std::vector<LineParameter>& DetectedLine, int nHeight, int nWidth){
	...
	float MaximumAngle = 15;//we're looking for vertical lines only for joint bars
	...
	//do Hough TF
	HoughTransform_Line(BinaryImageDst, MinimumAngle, AngleInterval_Coarse, NumberofAngles_Coarse, 0, 
		DistanceInterval_Coarse, NumberofDistances_Coarse, VoteTable_Coarse, nHeight, nWidth);
	...
	while (MaxVote > VoteThreshold)	{
		...
		//save all lines that meet minimum/maximum requirements
		LineParameter lp;
		lp.angle = Angle_withMaxVote_Coarse;
		lp.distance = Distance_withMaxVote_Coarse;
		if (lp.angle >= MinimumAngle && lp.angle <= MaximumAngle) {
			DetectedLine.push_back(lp);
		}
...
void HoughTransform_Line(unsigned char* BinaryImageDst, float MinAngle, float AngleInterval, 
	int No_Angles, float MinDistance, float DistanceInterval, int No_Distances, unsigned int** VoteTable, int nHeight, int nWidth){
		...
		for (int y = 0; y < nHeight; y++)
		{
			if (*(BinaryImageDst + y * nWidth + x) == 255)
			{
				for (int i = 0; i < No_Angles; i++)
				{
					...
					float Distance = ((float)x) * cos(Angle / 180 * 3.1415926) + ((float)y) * sin(Angle / 180 * 3.1415926);
