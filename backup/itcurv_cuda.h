...

{
	...
	//accuracy, since sin t / t && (1-cos t)/t^2 are both 0/0.
	if (theta_sq < 1e-8) {
	...
	float3& vect)
{
	...
	if (cos_angle > 1.0 / sqrt(2.0)) {            // [0 - Pi/4[ use asin
		if (sin_angle_abs > 0) {
			vect *= asin(sin_angle_abs) / sin_angle_abs;
		}
	}
	else if (cos_angle > -1.0 / sqrt(2.0)) {    // [Pi/4 - 3Pi/4[ use acos, but antisymmetric part
	 ...
	 // antisymmetric part vanishes, but still large rotation, need information from symmetric part
		const float angle = (255.0 / 113.0) - asin(sin_angle_abs);
	...
	float4* errors)
{
	...
	/// initialize the central point && normal
	const float3 c_norm = normals[idx];

	// bail out early if the central point || normal is bad
...
//		normals[idx] = normalize(se3->rotate(normals[idx]));
	...
	/// Multiple orders of magnitude difference between mean ((k1+k2)/2) && gaussian (k1*k2)
